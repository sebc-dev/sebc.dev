...
must_haves:
  truths:
    - "Pillar tags (IA, Ingenierie, UX) are indexed by Pagefind as tag filters"
    - "About page i18n keys exist in both EN and FR for all text placeholders"
    - "PillarBlock component renders a clickable card with icon, title, description, and link to search pre-filtered by pillar tag"
  artifacts:
    - path: "src/i18n/ui.ts"
      provides: "about.* translation keys for hero, pillar blocks, and CTA"
      contains: "about.hero"
    - path: "src/layouts/ArticleLayout.astro"
      provides: "Pagefind pillar tag filter meta tags"
      contains: "pillarTags"
  ...
  key_links:
    ...
    - from: "src/layouts/ArticleLayout.astro"
      to: "Pagefind index"
      via: "data-pagefind-filter meta tags for pillarTags"
...
Purpose: Provide the i18n strings, search integration, and reusable component needed for the About page build. The pillar tag indexing is critical -- without it, pillar block links on About will lead to zero search results.
...
Place this IMMEDIATELY after the existing tags mapping (around line 60), before the Pagefind metadata section.
  ...
  <verify>
Run `npm run typecheck` to confirm no type errors from the new i18n keys and ArticleLayout changes. Run `npx vitest run` to ensure existing tests pass.
  </verify>
  <done>
...
3. `src/i18n/ui.ts` contains `about.description`, `about.hero.tagline`, `about.hero.intro`, `about.pillar.ia.title`, `about.pillar.ia.description`, `about.pillar.engineering.title`, `about.pillar.engineering.description`, `about.pillar.ux.title`, `about.pillar.ux.description`, `about.cta`, `about.photo.alt` in both EN and FR
