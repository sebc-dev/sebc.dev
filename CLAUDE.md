...
**Astro 6 static site** deployed on Cloudflare Workers. Bilingual (EN default + FR), all routes locale-prefixed (`/en/...`, `/fr/...`).
...
- `import { z } from 'astro/zod'` NOT `from 'zod'` (Zod 4 in Astro 6)
...
- Cloudflare adapter v13: `main` → `@astrojs/cloudflare/entrypoints/server`, `platformProxy` removed
- Wrangler config: `assets.directory` NOT `pages_build_output_dir`
...
- **Push to main**: Deploy (`deploy.yml`) — build + Cloudflare Workers deploy via wrangler
